...
The `home-routes` handler in the `monaco.routes.home` namespace
...
The `home-page` function will in turn call the `monaco.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `monaco.handler` namespace:
...
Request middleware functions are located under the `monaco.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `monaco.dev-middleware` namespace found in
...
* Let `mount` know to start the XTDB node by `require`-ing `monaco.db.core` in some other namespace.
